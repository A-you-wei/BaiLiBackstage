...
def Mainfunction(page1):
    fromdata = {'page': str(page1), 'rp': '20', 'usepager': 'true', 'table_name': 't_246', 'fields': Field246}
    ...
    for data in json.loads(response.text)['rows']:
        select1 = "select id from hc_bl_overseas_education_background where id='" + str(data['id']) + "'"
        if sqlFeatures1(select1):
            print('Data already exists')
        else:
            strs2, strs3 = funData(Field246,data)
            sql1 = "insert into hc_bl_overseas_education_background(" + str(strs3) + ")values(" + str(strs2) + ")"
            print(sql1)
            sqlFeatures1(sql1)
